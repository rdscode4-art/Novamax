...
export default function JoinApplicationsPage() {
  const [activeTab, setActiveTab] = useState('volunteer'); // 'volunteer' | 'subadmin'
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const params = { page, limit: 10, formType: activeTab };
      if (debouncedSearch) params.search = debouncedSearch;
      if (filterStatus) params.status = filterStatus;
      
      const res = await api.get('/join-applications', { params });
  ...
  const updateStatus = async (id, status) => {
    ...
    try {
      await api.patch(`/admin/join-applications/${id}`, { status });
  ...
  const handleApprove = async (e) => {
    ...
    try {
      ...
      const res = await api.post(`/admin/join-applications/${approveModal}/approve`, formData);
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/admin/join-applications/${id}`);
  ...
  const columns = [
    { key: 'fullName', label: 'Applicant Name', render: (v) => <span style={{ fontWeight: 600 }}>{v}</span> },
    { key: 'phone', label: 'Phone' },
    { key: 'postLevel', label: 'Post Level', render: (v) => <span style={{ textTransform: 'capitalize' }}>{v}</span> },
    ...(activeTab === 'subadmin' ? [{ key: 'designation', label: 'Designation', render: (v) => <span style={{ textTransform: 'capitalize' }}>{v}</span> }] : []),
    ...
    { key: '_id', label: 'Actions', render: (_, row) => (
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={() => setViewItem(row)} style={btnStyle('#eff6ff', '#2563eb', '#bfdbfe')}>View Details</button>
  ...
  return (
    <div>
      <PageHeader title="📋 Join Applications" subtitle={`${pagination.total} total ${activeTab} applications`} />

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, borderBottom: '2px solid #e2e8f0' }}>
        <button 
          onClick={() => { setActiveTab('volunteer'); setPagination({ ...pagination, page: 1 }); }}
          style={{ ...tabStyle, ...(activeTab === 'volunteer' ? activeTabStyle : {}) }}
        >
          🙋‍♂️ Volunteer Applications
        </button>
        <button 
          onClick={() => { setActiveTab('subadmin'); setPagination({ ...pagination, page: 1 }); }}
          style={{ ...tabStyle, ...(activeTab === 'subadmin' ? activeTabStyle : {}) }}
        >
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <Table columns={columns} data={data} loading={loading} emptyMsg={`No ${activeTab} applications found`} />
        <Pagination {...pagination} onPage={fetchData} />
      </div>

      <Modal open={!!viewItem} onClose={() => setViewItem(null)} title={`${viewItem?.formType === 'volunteer' ? 'Volunteer' : 'Sub-Admin'} Application Details`} size="lg">
        {viewItem && (
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              {viewItem.photo && (
                <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
                  <img src={viewItem.photo.startsWith('http') ? viewItem.photo : `http://localhost:5000${viewItem.photo}`} alt="Applicant" style={{ width: 100, height: 100, objectFit: 'cover', borderRadius: '50%', border: '3px solid #e2e8f0' }} />
                </div>
              )}
              <InfoRow label="Full Name" value={viewItem.fullName} />
              <InfoRow label="Phone" value={viewItem.phone} />
              {viewItem.whatsapp && <InfoRow label="WhatsApp" value={viewItem.whatsapp} />}
              <InfoRow label="Email" value={viewItem.email} />
              <InfoRow label="Post Level" value={<span style={{ textTransform: 'capitalize' }}>{viewItem.postLevel}</span>} />
              
              {/* Sub-Admin specific fields */}
              {viewItem.formType === 'subadmin' && (
                <>
                  <InfoRow label="Father's Name" value={viewItem.fatherName} />
                  <InfoRow label="Aadhaar" value={viewItem.aadhaar} />
                  <InfoRow label="Designation" value={<span style={{ textTransform: 'capitalize' }}>{viewItem.designation}</span>} />
                  <InfoRow label="State" value={viewItem.state} />
                  <InfoRow label="Pin Code" value={viewItem.pinCode} />
                </>
              )}

              <InfoRow label="Status"><StatusBadge status={viewItem.status} /></InfoRow>
              <InfoRow label="Address" value={viewItem.address} col="1 / -1" />
            </div>
      ...
      <Modal open={!!approveModal} onClose={() => setApproveModal(null)} title="Approve & Generate Portal Credentials" size="md">
        <form onSubmit={handleApprove}>
          <p style={{ fontSize: 14, color: '#374151', marginBottom: 16 }}>
            Approving this application will generate a Portal User account for the applicant. 
...
const tabStyle = { padding: '10px 20px', background: 'transparent', border: 'none', borderBottom: '3px solid transparent', fontSize: 14, fontWeight: 600, color: '#64748b', cursor: 'pointer', marginBottom: '-2px' };
